...
{pkgs, ...}: let
  ...
  # The three Talos cluster nodes, running the nix daemon behind sshd in a
  # DaemonSet (pi-talos-home-ops#66). Keys generated by the pod on first
  # start and persisted on the node's ex-Longhorn partition; regenerating
  # them means updating these literals.
  #
  # Collected with:
  #   kubectl -n nix-builder logs -l app=nix-builder | grep '^ssh-ed25519'
  talosPi501PublicHostKey = "c3NoLWVkMjU1MTkgQUFBQUMzTnphQzFsWkRJMU5URTVBQUFBSUJpL2hScWtwbEREUGsyYnI5T05xbElFdXNVbXNyWWJmTHB0T0FodmJzYVE=";

  talosPi502PublicHostKey = "c3NoLWVkMjU1MTkgQUFBQUMzTnphQzFsWkRJMU5URTVBQUFBSU5uTW1vTms0ZlpoRlp1YVZlM1hpRnVqdk41N0NZTm1hUWlycWVFaGxwR1c=";

  talosPi503PublicHostKey = "c3NoLWVkMjU1MTkgQUFBQUMzTnphQzFsWkRJMU5URTVBQUFBSU8rZEdCaExqQWUzMW9nUU9ZSHBOTEIydmY3QzQ5QWNPK1dKQmhNNGhNT0I=";
  ...
  config = {
    ...
    nix = {
      ...
      buildMachines = [
        ...
        {
          # talos-pi5-01. Same RPi5 silicon as torrey, so the same native
          # armv7l capability -- VERIFIED on these nodes rather than assumed:
          # CONFIG_COMPAT=y, and an arm32v7 image executes with ELF class 1
          # and 32-bit virtual addresses.
          hostName = "nix-builder-talos-pi5-01";
          systems = ["aarch64-linux" "armv7l-linux"];
          protocol = "ssh-ng";
          sshUser = "root";
          sshKey = "/etc/nix/builder_ed25519";

          # LOWER than torrey's 2. These nodes also run the Kubernetes
          # control plane, and etcd shares their NVMe; its failure mode under
          # load is fsync latency, which surfaces as leader elections rather
          # than as anything labelled "disk". One job per node keeps build
          # I/O well clear of that. Raise it only with etcd latency in view.
          maxJobs = 1;

          # Above nas-sdg's emulated 1, below torrey's 3: real ARM cores, but
          # a node that has a day job. nix prefers torrey and spills here.
          speedFactor = 2;

          supportedFeatures = ["big-parallel" "gccarch-armv7-a" "gccarch-armv8-a"];
          publicHostKey = talosPi501PublicHostKey;
        }
        {
          # talos-pi5-02. Same RPi5 silicon as torrey, so the same native
          # armv7l capability -- VERIFIED on these nodes rather than assumed:
          # CONFIG_COMPAT=y, and an arm32v7 image executes with ELF class 1
          # and 32-bit virtual addresses.
          hostName = "nix-builder-talos-pi5-02";
          systems = ["aarch64-linux" "armv7l-linux"];
          protocol = "ssh-ng";
          sshUser = "root";
          sshKey = "/etc/nix/builder_ed25519";
          ...
          supportedFeatures = ["big-parallel" "gccarch-armv7-a" "gccarch-armv8-a"];
          publicHostKey = talosPi502PublicHostKey;
        }
        {
          # talos-pi5-03. Same RPi5 silicon as torrey, so the same native
          # armv7l capability -- VERIFIED on these nodes rather than assumed:
          # CONFIG_COMPAT=y, and an arm32v7 image executes with ELF class 1
          # and 32-bit virtual addresses.
          hostName = "nix-builder-talos-pi5-03";
          systems = ["aarch64-linux" "armv7l-linux"];
          protocol = "ssh-ng";
          sshUser = "root";
          sshKey = "/etc/nix/builder_ed25519";
          ...
          supportedFeatures = ["big-parallel" "gccarch-armv7-a" "gccarch-armv8-a"];
          publicHostKey = talosPi503PublicHostKey;
        }
      ];
    };
    ...
    programs.ssh.extraConfig = ''
      ...
      Host nix-builder-talos-pi5-01
        HostName 172.29.51.11
        # 30022, not 22: Talos itself runs no sshd, so this is the
        # DaemonSet's own listener on the node's host network.
        Port 30022
        # root inside the CONTAINER, not on the node -- Talos has no such
        # login. Every key is pinned to `nix-daemon --stdio` by a forced
        # command, so this grants building and nothing else.
        User root
        IdentityFile /etc/nix/builder_ed25519
        IdentitiesOnly yes
        ConnectTimeout 4
        HostKeyAlias nix-builder-talos-pi5-01

      Host nix-builder-talos-pi5-02
        HostName 172.29.51.12
        # 30022, not 22: Talos itself runs no sshd, so this is the
        # DaemonSet's own listener on the node's host network.
        Port 30022
        # root inside the CONTAINER, not on the node -- Talos has no such
        ...
        IdentitiesOnly yes
        ConnectTimeout 4
        HostKeyAlias nix-builder-talos-pi5-02

      Host nix-builder-talos-pi5-03
        HostName 172.29.51.13
        # 30022, not 22: Talos itself runs no sshd, so this is the
        # DaemonSet's own listener on the node's host network.
        Port 30022
        # root inside the CONTAINER, not on the node -- Talos has no such
        ...
        IdentitiesOnly yes
        ConnectTimeout 4
        HostKeyAlias nix-builder-talos-pi5-03
