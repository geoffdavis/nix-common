{
  ...
  in {
    ...
    # shell inline. It reads the *consumer's* flake.lock in CWD, so the locked
    # rev — not this flake's version — is what lands in the workflows.
    # Body in scripts/sync-pin.sh; writeShellApplication shellchecks it at
    # build time, which the old inline '' '' string could not be.
    apps = forAllSystems (pkgs: {
      sync-pin = {
        type = "app";
        program = "${pkgs.writeShellApplication {
          name = "sync-pin";
          runtimeInputs = with pkgs; [coreutils diffutils findutils gnugrep gnused jq];
          text = builtins.readFile ./scripts/sync-pin.sh;
        }}/bin/sync-pin";
        meta.description = "Rewrite a consumer's nix-common workflow pins to match its flake.lock";
      };
    });
